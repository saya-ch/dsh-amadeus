...
不是对话框套壳。是**一场演出**：打字机逐字浮现、立绘随情绪切换、幕后活动在文本框上方滚动成字幕，
...
> 🎨 **全部美术素材（立绘 / 背景 / 装饰）均取自 DSH 社区开源项目，非本仓库原创**
> 署名链与许可 → [素材与合规](#-素材与合规)
...
flowchart TB
    subgraph HOST["🖥 电脑 · DSH Host（同一进程）"]
        ...
        PLUGIN["dsh-amadeus plugin<br/>① 接 agent 事件流 → Galgame 帧<br/>② 注册 extension 业务路由"]
...
<details>
<summary><b>⚙️ <code>src/</code> 模块清单（点击展开）</b></summary>
...
# 构建：lib/index.mjs（Host 插件）+ lib/client.js（web 控制面板）
npm run build
...
```bash
# 本地管理（DSH 跑着才有）
curl http://127.0.0.1:3080/api/amadeus/status
```
...
<br><sub>▲ 背景库：<b>深海宫殿夜景</b>（demo）+ 三张工作场景 · 居家室内待补，<a href="#-共创">欢迎共创</a></sub>
<br><sub>🎨 图片取自 <a href="#-素材与合规">DSH 社区开源项目</a>，非本仓库原创</sub>
</div>
...
- Host 会话闭环
  （建会话 → Agent 干活 → SSE 演出 → 报告/选项/审批全链路）
- App 剧场全部演出层
- 配对 / 认证 / TLS
  （LAN 直连 + cpolar 远程）
- 读档页按工作区组织
...
flowchart LR
    A["✅ 剧场闭环<br/>配对认证 · 远程隧道"]
    B["⏳ 开场白文案<br/>BGM 素材 · 专属 logo"]
    C["⏳ 表情差分<br/>动作 × 表情矩阵"]
    D["⏳ 居家背景库<br/>frpc 实机联调"]
    E["🔮 Live2D 接入<br/>SpriteRenderer 已抽象"]
...
- **专属 logo** — 现 🐳 emoji + 标题文字
- **表情差分** — 动作 × 表情矩阵，GPT image2 分批扩充，未开始
- **背景库** — 缺居家室内
...
```mermaid
flowchart LR
    F["① Fork"] --> B["② 开分支"] --> C["③ 改点东西"] --> P["④ Pull Request"]
```
...
| 对象 | 许可 | 说明 |
|:---|:---|:---|
| **代码** | Apache-2.0 | 见 `package.json` |
| **美术素材** | CC BY-NC-SA 4.0 | 署名 · 非商用 · 相同方式共享 |
| **字体**（马路口圆体） | SIL OFL 1.1 | `app/amadeus-android/app/src/main/assets/licenses/` |

> ⚠️ **CC BY-NC-SA 限非商业使用** —— 本项目为个人非商业项目，符合该条款；
> **若转商用，必须替换全部美术素材**。素材与代码协议分离：素材 CC BY-NC-SA，代码 Apache-2.0。

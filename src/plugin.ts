...
import { AMADEUS_MODE_CONFIG } from './amadeus-mode.js'
import { createAmadeusRoute, ensureTagForAmadeus } from './amadeus-gateway.js'
import { hasAmadeusTag } from './amadeus-tags.js'

/** Stable Cordis plugin name. Forked from dsh-mobile. */
...
// Amadeus: in-memory session stub (replace with real DSH session API when available)
const amadeusSessions = new Map<string, { id: string, title: string, mode: string, updatedAt: number }>()
function amadeusSessionStore() {
  return {
    list: async (mode: string) => [...amadeusSessions.values()].filter(s => s.mode === mode) as any,
    create: async (mode: string, title?: string) => {
      const id = Math.random().toString(36).slice(2, 10)
      const s = { id, title: title ?? `Amadeus 存档 ${amadeusSessions.size + 1}`, mode: 'amadeus' as const, updatedAt: Date.now() }
      amadeusSessions.set(id, s as any)
      return s as any
    },
    get: async (id: string) => amadeusSessions.get(id) ?? null,
  }
}
...
export async function apply(ctx: Context, config: PluginConfig): Promise<void> {
  ...
  // Amadeus: log mode config for verification (high privilege)
  // In real DSH, this would be ctx.modes.register(AMADEUS_MODE_CONFIG) + llm hook for tags
  // Here we ensure the gateway enforces tag protocol
  await ctx.effect(async () => {
    const amadeusRoute = createAmadeusRoute({ sessions: amadeusSessionStore() as any })
  ...
  }, 'dsh-amadeus: amadeus mode + independent LAN/remote + Galgame theatre')
}
